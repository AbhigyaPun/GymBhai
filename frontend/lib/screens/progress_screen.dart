...
class _ProgressScreenState extends State<ProgressScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      _memberGoal  = member?['goal'] ?? 'maintain';
      ...
      if (res.statusCode == 200) {
        ...
        setState(() { _error = 'Failed to load progress'; _loading = false; });
      }
    } catch (_) {
                  ...
                  delegate: SliverChildListDelegate([
                    ...
                    Row(children: [
                      Expanded(child: _StatCard(
                        ...
                        value: _profile?['current_weight'] != null
                      ...
                      Expanded(child: _StatCard(
                        label: 'Target Weight',
                        value: _profile?['target_weight'] != null
                    ...
                    Row(children: [
                      Expanded(child: _StatCard(
                        label: 'To Goal',
                        value: _profile?['weight_to_goal'] != null
                            ? '${_profile!['weight_to_goal']} kg'
                            : '—',
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    '${_profile?['progress_percentage'] ?? 0}%',
                                    style: TextStyle(
                                        fontSize: 14,
                                        fontWeight: FontWeight.bold,
                                        color: _goalColor),
                              ...
                              ClipRRect(
                                ...
                                child: LinearProgressIndicator(
                                  value: ((_profile?['progress_percentage']
                                              ?? 0) as num)
                                          .toDouble() /
                                      100,
                                  minHeight: 10,
                                  ...
                                  valueColor:
                                      AlwaysStoppedAnimation(_goalColor),
                                ),
                              ),
                            ],
                          ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_profile?['recommended_target'] != null)
                              Text(
                                'Recommended target for ${_goalLabel.toLowerCase()}: '
                                '${_profile!['recommended_target']} kg',
                                style: TextStyle(
                                    fontSize: 12,
                                    color: _goalColor,
                            ...
                            child: Column(
                              children: List.generate(_logs.length, (i) {
                                ...
                                if (i < _logs.length - 1) {
                                  ...
                                  if (diff > 0) change = '+${diff.toStringAsFixed(1)} kg';
                                  else if (diff < 0) change = '${diff.toStringAsFixed(1)} kg';
  ...
  void _showGoalsModal(BuildContext context) {
    final currentCtrl = TextEditingController(
        text: _profile?['current_weight']?.toString() ?? '');
    final targetCtrl  = TextEditingController(
        text: _profile?['target_weight']?.toString() ?? '');
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                if (recommended != null)
                  Text(
                    'Recommended target for $_goalLabel: $recommended kg',
                    style: TextStyle(
                        fontSize: 12,
                        color: _goalColor,
                        fontStyle: FontStyle.italic),
                ...
                const SizedBox(height: 4),
                // Quick set recommended
                if (recommended != null)
                  GestureDetector(
                    onTap: () => targetCtrl.text = '$recommended',
                    child: Text(
                      'Use recommended: $recommended kg',
                      style: TextStyle(
                          fontSize: 12,
                          color: _goalColor,
                ...
                const SizedBox(height: 12),
                _ModalField(
                    label: 'Height (cm)', controller: heightCtrl),
                const SizedBox(height: 20),
                ElevatedButton(
                  onPressed: saving
                      ? null
                      : () async {
                          ...
                          try {
                            ...
                            final res   = await http.put(
                              Uri.parse(
                                  '${AppConfig.apiBaseUrl}/progress/profile/'),
                              headers: {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                _ModalField(
                    label: 'Weight (kg)', controller: weightCtrl),
                _ModalField(
                    label: 'Notes (optional)', controller: notesCtrl),
                const SizedBox(height: 20),
                ElevatedButton(
                  onPressed: saving
                      ? null
                      : () async {
                          ...
                          try {
                            ...
                            final res   = await http.post(
                              Uri.parse(
                                  '${AppConfig.apiBaseUrl}/progress/logs/'),
                              headers: {
                                'Authorization': 'Bearer $token',
                                'Content-Type': 'application/json',
                              },
...
class _WeightChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final weights = logs.reversed
        .map((l) => double.tryParse('${l['weight']}') ?? 0.0)
        .toList();

    final allValues = [...weights, if (targetWeight != null) targetWeight!];
    ...
    for (int i = 0; i < weights.length; i++) {
      ...
      final y = size.height -
          (weights[i] - minY) / (maxY - minY) * size.height;
    ...
    if (targetWeight != null) {
      final ty = size.height -
          (targetWeight! - minY) / (maxY - minY) * size.height;
